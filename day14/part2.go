...
func placeSand(x, y int, grid *map[Field]bool, minY int) bool {
	isElementAtPos := (*grid)[Field{x, y}]
	if isElementAtPos && y == 0 {
		return false
	}
	isElementAtNextPos := (*grid)[Field{x, y + 1}]
	if isElementAtNextPos || minY < y {
		isElementAtLeft := (*grid)[Field{x - 1, y + 1}] || minY < y
		if !isElementAtLeft {
			return placeSand(x-1, y, grid, minY)
		}
		isElementAtRight := (*grid)[Field{x + 1, y + 1}] || minY < y
		if !isElementAtRight {
			return placeSand(x+1, y, grid, minY)
		}
		(*grid)[Field{x, y}] = true
