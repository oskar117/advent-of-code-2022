...
func main() {
	...
	b := (1 << len(nonEmptyValves)) - 1

	max := 0

	for i := 0; i < (b + 1) / 2; i++ {
		if m := getMaxPressure(26, "AA", i, valves, dists, indicies) + getMaxPressure(26, "AA", b ^ i, valves, dists, indicies); m > max {
			max = m
		}
	}
	fmt.Println("Max pressure is", max)
}
